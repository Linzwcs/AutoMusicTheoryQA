...
class BarLinePlacementQuestion(BasePrototype):
    ...
    beats = ["2/4","2/2","3/4", "3/2", "4/4", "4/2", "9/8"]
    
    @staticmethod
    def _insert_barlines(beats: list, notes_with_duration: list, measure_duration: Fraction) -> str:
        """
        ...
        for beat in beats:
            ...
            for i in range(token_nums):
                ...
                note_val = Fraction(base_frac*note_obj.duration.quarterLength)
                cumulative += note_val
            
            if current_measure_val + cumulative > measure_duration + Fraction(1, 64): # 使用容差
            ...
            if abs(current_measure_val - measure_duration) < Fraction(1, 64): # 使用容差
                barred_tokens.append('|')
                current_measure_val = Fraction(0)
            idx+=token_nums
    ...
    def produce( music_sheet: MusicSheet, n_measure=4):
        ...
        original_measures_str = music_sheet.get_first_n_measure(n_measure,drop_chord=True)
        if not original_measures_str: return None
        
        dechorded_measures_str = re.sub(r'\[([A-Ga-gz][,\']*\d*/?\d*) [^\]]*\]', r'\1', original_measures_str)
        ...
        correct_barred_body = BarLinePlacementQuestion._insert_barlines(beats, notes_and_rests, Fraction(ts.ratioString))
        ...
        try:
            possible_wrong_ts_durations.remove(ts.ratioString)
        except ValueError:
            pass
        ...
        for wrong_duration in possible_wrong_ts_durations:
            ...
            distractor_body = BarLinePlacementQuestion._insert_barlines(beats, notes_and_rests, wrong_duration)
        ...
        return MusicTheorySingleChoiceQuestion(
            ...
            catagory=BarLinePlacementQuestion.catagory,
